...
class Rect:
    def __init__(self, canvas, x, y, *args, **kwargs):
        self.canvas = canvas
        self.id = canvas.create_rectangle(b2x(x), b2y(y), b2x(x + 1), \
            b2y(y + 1), *args, **kwargs)
        self.x = x
        self.y = y
    ...
    def redraw(self):
        self.canvas.coords(self.id, b2x(self.x), b2y(self.y), \
            b2x(self.x + 1), b2y(self.y + 1))
...
class Apple(Sprite):
    ...
    def random_location(self):
        return random.randint(0, GRID_COLS - 1), \
...
class Game:
    def __init__(self):
        ...
        l = tk.Label(frame, textvariable=self.score1_svar, fg=PLAYER1_COLOR, \
            bg=SCORE_BG_COLOR)
        l.pack(side=tk.LEFT, expand=False)
        self.label1 = l
        self.score2_svar = tk.StringVar()
        l = tk.Label(frame, textvariable=self.score2_svar, fg=PLAYER2_COLOR, \
            bg=SCORE_BG_COLOR)
        ...
        # play area
        self.canvas = tk.Canvas(self.master, width=SCREEN_WIDTH, \
            height=SCREEN_HEIGHT, highlightthickness=0, bg=BG_COLOR)
        ...
        self.snake2 = Snake(self, PLAYER2_KEYS, inity=GRID_ROWS - 1, \
            color=PLAYER2_COLOR)
    ...
    def resize(self, event):
        """
        Handle resize events so that the drawn objects adapts to the window
        size.
        """
    ...
    def update(self):
        self.causes_of_death = set([(r.x, r.y) for r in self.snake1.rects] + \
            [(r.x, r.y) for r in self.snake2.rects])
